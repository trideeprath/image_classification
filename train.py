...
# log file directory where the tf logs and chkpts are saved
log_dir = 'log'
...
items_to_descriptions = {'image': 'A 3-channel RGB coloured flower image that is either tulips, '
                                  'sunflowers, roses, dandelion, or daisy.',
...
def get_split(split_name, dataset_dir, file_pattern=file_pattern, file_pattern_for_counting='flowers'):
    ...
    This function will set up the decoder and dataset information all into one Dataset class so that you can avoid
    the brute work later on. Your file_pattern is very important in locating the files later.
    ...
    dataset = slim.dataset.Dataset(data_sources=file_pattern_path, decoder=decoder, reader=reader,
                                   num_readers=4, num_samples=num_samples, num_classes=num_classes,
...
def load_batch(dataset, batch_size, height=image_size, width=image_size, is_training=True):
    ...
    # Batch up the image by enqueing the tensors internally in a FIFO queue and
    # dequeueing many elements with tf.train.batch.
    images, raw_images, labels = tf.train.batch([image, raw_image, label], batch_size=batch_size, num_threads=4,
                                                capacity=4 * batch_size, allow_smaller_final_batch=True)
...
def run():
    ...
    with tf.Graph().as_default() as graph:
        ...
        def train_step(sess, train_op, global_step):
            """
            Simply runs a session for the three arguments provided and gives a logging on the time elapsed for
            each global step
            """
